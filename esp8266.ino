...
String computerIp = "192.168.15.4";
...
bool hasGoneBack = true;

void loop() {
  webSocket.loop();
sensors_event_t event; 
 accel.getEvent(&event);
  posit = event.acceleration.y;
 
  
if(digitalRead(btn1) == 1){
  Serial.println("btn1");
   webSocket.sendTXT("btn1");
  delay(300);
}
if(digitalRead(btn2) == 1) {
  Serial.println("btn2");
   webSocket.sendTXT("btn2");
  delay(300);
}
if(digitalRead(btn3) == 1) {
  Serial.println("btn3");
   webSocket.sendTXT("btn3");
  delay(300);
}
if(digitalRead(btn4) == 1) {
  Serial.println("btn4");
   webSocket.sendTXT("btn4");
  delay(300);
}

 if(posit>11 && hasGoneBack == true){
